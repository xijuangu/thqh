...
#define SMSDK_ERR_USER_LOCKED             (SMSDK_ERR_BASE + 0x0409)     /* 用户登录错误次数过多，账户已锁定 */
...
/*
说明：
1. 本地根证路径：./smidata${BrokerName}/ifsca-cert.cer（证书内容为BEGIN/END的PEM格式）
如：
	假设BrokerName为空
	    Linux: ./smidata/ifsca-cert.cer
	    Windows: .\smidata\ifsca-cert.cer
	假设BrokerName为infosec
	    Linux: ./smidatainfosec/ifsca-cert.cer
	    Windows: .\smidatainfosec\ifsca-cert.cer
2. 本地证书目录：用户数据目录\smidata${BrokerName}\
如：
	假设BrokerName为空
	    Linux: ~/ifs/smidata/
	    Windows: %AppData%\ifs\smidata\
	假设BrokerName为infosec
	    Linux: ~/ifs/smidatainfosec/
	    Windows: %AppData%\ifs\smidatainfosec\

*/
/* 证书、签名验签配置 */
typedef struct SMUserConfig_s {
    const char  *BrokerID;      /* 可空，用户的 broker id，若期货公司不包含broker_id可不传 */
    const char  *BrokerName;    /* 可空，用户的期货公司名称，只支持数字与字母，若期货公司不包含broker_id可不传 */
    const char  *UserID;        /* 非空，用户的 user id */
    const char  *Pin;           /* 非空，用户的本地PIN码 */
    const char  *Password;      /* 非空，用户的密码，用于用户下证校验 */
    const char  *CertHost;      /* 非空，协同签名服务器地址 */
    int          CertPort;      /* 非空，协同签名服务端端口 */
    int          CertSocket;    /* 可空，协同签名服务socket */
    int          TimeoutMs;     /* 非空，连接超时时间 */
    const char  *ServiceID;     /* 非空，MAuth 服务标识 */
    const char  *AppID;         /* 非空，MAuth AppID */
    const char  *SecretKey;     /* 非空，MAuth SecretKey */
    int          NoSync;        /* 可空，关闭接口阻塞：为0时，接口阻塞直至接口调用完成；非0时，套接字或文件IO阻塞时接口保存状态立即返回并等待下一次调用 */
} SMUserConfig_t;
 ...
 * Windows平台，用户证书存放在"%AppData%\ifs\smidata\"目录下
 * Linux平台，用户证书存放在"$HOME/ifs/smidata/"目录下
 *
 ...
 *    成功：继续3）
 * 3) 检查证书状态：
 *    证书无效：返回SMSDK_ERR_CERT_INVALID，客户端调用SMCertApi的SMCertSDK_CertEnroll，重新申请证书
 *    证书过期：返回SMSDK_ERR_CERT_EXPIRED，客户端调用SMCertApi的SMCertSDK_CertDelay，更新证书
 *    有效：继续4）
 * 4）协同签名校验私钥等；
 *    失败：返回SMSDK_ERR_INTERNAL开头的错误码，报错
 *    成功：返回SMSDK_ERR_NONE
 *
 * @param hSDK  [in] SDK句柄
 * @return int  成功返回 SMSDK_ERR_NONE
