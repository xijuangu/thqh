...
#define SMCERTSDK_ERR_CERT_NOT_EXISTS   (SMCERTSDK_ERR_BASE + 0x0403)     /* 证书不存在 */
...
#define SMCERTSDK_ERR_CERT_INVALID      (SMCERTSDK_ERR_BASE + 0x0406)     /* 证书无效 */
...
#define SMCERTSDK_ERR_USER_LOCKED       (SMCERTSDK_ERR_BASE + 0x0409)     /* 用户登录错误次数过多，账号已锁定 */
...
typedef struct SMCert_s {
	...
	int IsCurrent;     /* 是否为当前设备证书 */
} SMCert_t;

// P7签名方式
typedef enum
{
    SM_ATTACH_DATA_AND_CERT = 0,   // 包含原文和证书
    SM_DETACH_DATA          = 0x1, // 不包含原文
    SM_DETACH_CERT          = 0x2, // 不包含证书
    SM_DETACH_CERT_AND_DATA = 0x3, // 不包含原文和证书
    SM_DETACH_CERT_AND_RANDOM = 0x4, // 不带证书且获取服务端随机数作为签名原文的一部分
    SM_ATTACH_CERT_AND_SKSRANDOM = 0x8, // 带证书和原文且获取协同网关服务端随机数作为签名原文的一部分
} SM_SIGN_FLAG;
 ...
 * @return const char * 
 */
EXPORT_C_API const char * SMCertSDK_GetVersion();
 ...
 *                 失败返回错误码
 */
EXPORT_C_API int SMCertSDK_Init(const char *LogFile);
 ...
 *              失败返回错误码
 */
EXPORT_C_API int SMCertSDK_Clean(void);
 ...
 *                失败返回错误码
 */
EXPORT_C_API int SMCertSDK_New(const SMCertUserConfig_t *Config, SMCertSDK_t *hSDK);
 ...
 *              失败返回错误码
 */
EXPORT_C_API int SMCertSDK_Free(SMCertSDK_t hSDK);
...
*
*/
EXPORT_C_API int SMCertSDK_CertEnroll(SMCertSDK_t hSDK);
 ...
 *
 * 国密厂商API内需校验PIN（存储于hSDK句柄中），若PIN校验错超过10次，锁定PIN码，返回SMCERTSDK_ERR_PIN_LOCKED错误码
 *
 * @param hSDK		[in] SDK句柄
 * @return int  成功返回 SMCERTSDK_ERR_NONE
 *              网络IO阻塞返回 SMSSLCERT_ERROR_WANT_READ/SMSSLCERT_ERROR_WANT_WRITE
 ...
 *
 */
EXPORT_C_API int SMCertSDK_CertDelay(SMCertSDK_t hSDK);
 ...
 *
 */
EXPORT_C_API int SMCertSDK_CertQuery(SMCertSDK_t hSDK, const SMCert_t **pCert, int *total);
 ...
 *
 */
EXPORT_C_API int SMCertSDK_CertRevoke(SMCertSDK_t hSDK, const char *CertID);
...
*
*/
EXPORT_C_API int SMCertSDK_ResetPin(SMCertSDK_t hSDK, const char *NewPIN);
...
*
* 该接口有网络操作
* 做身份认证时传入的flag取值为SM_ATTACH_CERT_AND_SKSRANDOM，参数b64OriginData需要传入用户账号
* 内部会先校验用户pin
*
* @param hSDK                  [in]     SDK句柄
* @param b64OriginData         [in]     签名原文的Base64编码
* @param flag                  [in]     生成pcks7数据flag 详见 { @link SIGN_FLAG }
* @param b64Pkcs7MsgBuf        [out]    存放签名消息Base64编码的缓冲
* @param b64Pkcs7MsgBufLen     [IN,OUT] 输入b64Pkcs7MsgBuf缓冲区的大小，返回实际需要的大小
* @return int  成功返回 SMCERTSDK_ERR_NONE
*              失败返回错误码
*
*/
EXPORT_C_API int SMCertSDK_SignMessage(SMCertSDK_t hSDK, const char* b64OriginData, SM_SIGN_FLAG flag, char* b64Pkcs7MsgBuf, int* b64Pkcs7MsgBufLen);
...
* @param hSDK            [in] SDK句柄
* @param b64OriginData   [in] 签名原文的Base64编码
* @param b64Cert         [in] 证书base64数据，传入签名数据是不带证书签名数据时需要传入
* @param b64Pkcs7Message [in]  PKCS#7格式的签名数据Base64编码后的字符串
*
* @return = 0 成功
*         !=0 错误码
*/
EXPORT_C_API int SMCertSDK_VerifyMessageOnline(SMCertSDK_t hSDK, const char* b64OriginData, const char* b64Cert, const char* b64Pkcs7Message);
