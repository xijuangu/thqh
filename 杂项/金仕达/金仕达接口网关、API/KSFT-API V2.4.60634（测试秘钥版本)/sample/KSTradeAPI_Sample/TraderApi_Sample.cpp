...
{
public: 
	CTraderApiSample(CThostFtdcTraderApi *pUserApi, TThostFtdcBrokerIDType chBrokerID, TThostFtdcUserIDType chUserID, TThostFtdcPasswordType chPassword, TThostFtdcContractCodeType chContract, bool GM_ = false)
	{
		m_nRequestID = 0;
		m_pUserApi = pUserApi;
	...
	virtual void OnFrontConnected()
	{
		...
		// 商密通道 客户端登录时，需要调用新的 ReqUserLoginSM 接口，没有新增 rsp 接口，复用之前登录应答接口 OnRspUserLogin
		if(GM)
		{
			CThostFtdcReqUserLoginSMField reqUserLoginSM;
			memset(&reqUserLoginSM, 0, sizeof(reqUserLoginSM));
			strcpy(reqUserLoginSM.AppID,"bassss");

			//strcpy(reqUserLoginSM.AppID,"KINGSTAR_NewMidas_8.2.1");
			//strcpy(reqUserLoginSM.AuthCode,"f1c2ec5f7418c4458958ee40e8d19893");
			strcpy(reqUserLoginSM.AuthCode,"3e2887d177d3dd575d37fd7e2f8c584e");
                                            //
			strcpy(reqUserLoginSM.MAuthServiceID ,"JSD");
			strcpy(reqUserLoginSM.MAuthAppID ,"APP_SDK_TEST");
			strcpy(reqUserLoginSM.MAuthSecretKey ,"Bx3NF8Z9vjadPyqH7Bpz+JINvHU0FGJWMggIX0UJXCVW1geEMAthM/D6EJpcsDY8ymXwpDGcAs24PFPc3sD36Q==");

			strcpy(reqUserLoginSM.UserID, m_chUserID);
			strcpy(reqUserLoginSM.BrokerID, m_chBrokerID);
			strcpy(reqUserLoginSM. BrokerID, m_chBrokerID);
			strcpy(reqUserLoginSM.UserID, m_chUserID);
			strcpy(reqUserLoginSM.Password, m_chPassword);

			strcpy(reqUserLoginSM.PIN, "123456");

			m_pUserApi->ReqUserLoginSM(&reqUserLoginSM,m_nRequestID++);
		}
		// 以前的认证
		else
		{
			...
			strcpy(reqAuthenticateField.AppID,"Kingstar_ITS_V2.6.0");
			strcpy(reqAuthenticateField.AuthCode,"20d63ff452c6b9686f3b9733d5464ddf");
			strcpy(reqAuthenticateField.UserID, m_chUserID);
			strcpy(reqAuthenticateField.BrokerID, m_chBrokerID);
...
class CTestKSVocSpi : public CKSVocSpi
{
public:
	CTestKSVocSpi(){}
	~CTestKSVocSpi() {}
	...
	{
		printf("OnRspWithdrawCreditApply:");
		if (NULL != pRspQryWithdrawCredit)
		{
			printf("当日出金额度 = %lf", pRspQryWithdrawCredit->WithdrawCredit);
		}
		printf("\n");
		printf("ErrorCode=[%d], ErrorMsg=[%s]\n", pRspInfo->ErrorID, pRspInfo->ErrorMsg);
		printf("RequestID=[%d], Chain=[%d]\n", nRequestID, bIsLast);
	...
	virtual void OnRspQryWithdrawReservedApply(CKSRspWithdrawReservedApplyField *pRspWithdrawReservedApply, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
	{
		...
		printf("OnRspQryWithdrawReservedApply:");
		if (NULL != pRspWithdrawReservedApply)
		{
			printf("银行名称位%s", pRspWithdrawReservedApply->BankName);
		}
		printf("\n");
		printf("ErrorCode=[%d], ErrorMsg=[%s]\n", pRspInfo->ErrorID, pRspInfo->ErrorMsg);
		printf("RequestID=[%d], Chain=[%d]\n", nRequestID, bIsLast);
	...
	// 出入金响应
	virtual void OnRspQryDepositAndWithdraw(CKSRspDepositAndWithdrawField *pRspQryDepositAndWithdraw, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
	{
		printf("OnRspQryDepositAndWithdraw:");
		if (NULL != pRspQryDepositAndWithdraw)
		{
			printf("入金金额%f", pRspQryDepositAndWithdraw->Deposit);
		}
...
int main(int argc, char* argv[])
{
	...
	CKSVocApi *pVocAPI = NULL;
	//CKSVocApi *pTestVocAPI = NULL;
	for (int i=0; i < MAX_CONNECTION; i++ )
	{
		...
		pSpi[i] = new CTraderApiSample(pUserApi[i], "", "102222", "1", "IF1506",GM);

		CCosHandler CosSpiTest;			//产生一个条件单响应的实例

		if (NULL == pUserApi[i])
		{
			printf("KSInterB2C.lkc is not exist! Please check the running path.\n");
			break;
		}
		// 注册spi实例
		...
		// 注册前置机	
		// 运行此程序前请修改 IP端口
		//格尔商密地址格式：gmsmk://SSLIP:SSLPORT;protocol://CERTHOST:CERTPORT
		pUserApi[i]->RegisterFront("gmsmk://10.254.5.55:50443;protocol://10.254.5.55:18555");
                                //如果只有一个硬件厂商，不需要同时都注册RegisterFront
		//信安商密地址格式：gmsmi://SSLIP:SSLPORT;protocol://CERTHOST:CERTPORT
		pUserApi[i]->RegisterFront("gmsmi://10.253.11.11:9443;protocol://10.253.11.11:8443");
		pUserApi[i]->RegisterFront("tcp://140.206.157.244:10010");
		// 门户接口用法
		...
		getchar();
		// 需要等登录成功后，才能执行如下操作
		pVocAPI = (CKSVocApi *) pUserApi[i]->LoadExtApi(static_cast<CKSVocSpi*>(pSpi[i]),KS_VOC_API);

		// 出入金查询请求
		CKSQryDepositAndWithdrawField qry;
		memset(&qry, 0, sizeof(CKSQryDepositAndWithdrawField));
		strcpy(qry.BrokerID, "31000853");
		strcpy(qry.DateStart, "20221102");
		strcpy(qry.DateEnd, "20221102");
		qry.FundDirection = THOST_FTDC_FD_In;
		strcpy(qry.CurrencyID, "RMB");
		pVocAPI ->ReqQryDepositAndWithdraw(&qry, pSpi[i]->m_nRequestID++);
